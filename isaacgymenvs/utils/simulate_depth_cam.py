...
if __name__ == "__main__":
    ...
    if load_pcd:
        # points = np.load("box_env_pcd100.npy")
        points = np.load("box100_env_pcd20k.npy")

        pcd = torch.tensor(points, device=device)
        pcd = pcd.repeat((5, 1, 1))
        BATCH = pcd.shape[0]

        # 1. vision goes mainly from the right side of the arm
        # 2. assume camera largely point towards the object position (so randomize view center around the object)
        cam_pos, view_dirs = sample_cameras(BATCH, cam_target_pos=torch.tensor([0,0,0]*BATCH, device=device), device=device, dtype=pcd.dtype)
        ...
        if render:
            # depth, pcd_world, valid = render_points_to_world_grid(
            #     pcd.to(device), # (B, N, 3)
            #     view_dirs.to(device), # (B, 3), just normalize
            #     cam_pos.to(device), # (B, 3), xyz cam pos
            #     H=848//4, W=480//4,
            #     fov_x_deg=87.0, # intel 455
            #     fov_y_deg=58.0, # intel 455
            #     inflate_px=2, # 'pooling' kernal size, 2 is 5x5
            #     clip_mode="post", # just use this
            #     near_m=0.4, # depth cut-off
            #     far_m=6.0, # depth cut-off
            #     jitter_std_m=0.004, # noise
            #     jitter_mode="xyz", # tangent or xyz
            # )
    ...
    print("Hertz:", 1/(time.time() - start_time))

    # Visualize item i
    i = 0

    if render:
        pcd_full_i = pcd[i]                                       # (N,3)
        pcd_view_i = pcd_world[i][valid[i]].view(-1,3)            # (M,3)

    vis = visualize_meshcat(
        pcd_full_i, pcd_view_i,
        cam_pos[i], view_dirs[i],
        point_size=0.012,
    )
    print("Number of full points", pcd_full_i.shape)
    print("Number of rendered points", pcd_view_i.shape)
