...
class FrankaLEAPPickSide(FrankaLEAP):
    ...
    def _post_init_buffers(self):
        ...
        if self.eef_init["enable"]:
            ...
            # self.canonical_joint_config[:, :7] = torch.tensor([[-0.5*np.pi, -0.25*np.pi, 0.0, -0.75*np.pi, 0.5*np.pi, 0.5*np.pi, 0.0]]*self.num_envs, device=self.device) # for debugging purposes
