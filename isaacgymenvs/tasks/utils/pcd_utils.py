...
from isaacgymenvs.tasks.utils.geometry import ObjaMesh, construct_mixed_point_cloud
...
def has_object_in_hand(scene_pcd_params):
    ...
    expected_params_with_object = 33 * points_per_object + 11 + 1
...
def compute_robot_pcd(
    ...
    if has_in_hand:
        in_hand_params = scene_pcd_params[-11:]
        in_hand_pcd = compute_in_hand_pcd(configs.cpu().numpy(), num_in_hand_points, in_hand_params)
        scene_pcd_params = scene_pcd_params[:-11]
        robot_pcd = torch.cat(
            (robot_pcd, torch.from_numpy(in_hand_pcd).to(robot_pcd.device)), dim=1
        )

    robot_pcd = vectorized_subsample(robot_pcd, dim=1, num_points=num_robot_points)
    return robot_pcd
