...
from isaacgymenvs.model.drp_pcd_encoder import DRPPointNetEncoder
...
class FrankaLEAPPick(FrankaLEAP):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        self.pcd_encoder = DRPPointNetEncoder.from_pretrained("jimyoung6709/DRP_Scene_PCD_Encoder")
        self.pcd_encoder = self.pcd_encoder.half().to(sim_device)
    ...
    def compute_observations(self):
        ...
        with torch.no_grad():
            with torch.autocast('cuda', dtype=torch.float16):
                # mean pooling of the tokens
                pcd_static_latent = self.pcd_encoder(self.static_pcds).mean(dim=1)
                pcd_object_latent = self.pcd_encoder(self.object_pcds).mean(dim=1)

        pcd_latent = torch.cat([pcd_static_latent, pcd_object_latent], dim=-1) # 124*2
