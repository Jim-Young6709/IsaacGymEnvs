...
class Curobo(MotionPlannerBase):
    ...
    def get_actions_open_loop(self, env_obs_dict):
        ...
        for i in range(self.num_envs):
            if self.allow_replanning or self.env_planning_success_flag[i]:
    ...
    def get_actions(self, env_obs_dict):
        ...
        if self.normalize_speed:
            ...
            interpolated = F.interpolate(segment, size=num_interp_steps, mode='linear', align_corners=True)
            # (15, num_envs, 7)
            interpolated = interpolated.permute(2, 0, 1)
            return interpolated
        else:
            start_id = 5
            return planning_actions_abs[start_id:start_id+15]
