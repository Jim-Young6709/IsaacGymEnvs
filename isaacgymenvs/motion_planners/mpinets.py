...
class MPiNets(MotionPlannerBase):
    ...
    def set_up_policy(self):
        ...
        checkpoint_name = "mpinets.ckpt"
        checkpoint_file = Path(checkpoint_folder) / checkpoint_name

        self.model = MotionPolicyNetwork.load_from_checkpoint(checkpoint_file).cuda()
        self.model.eval()
    ...
    def get_actions(self, env_obs_dict):
        ...
        nn_pcd_obs = self._prepare_mpinets_observation(
    ...
    def get_actions_open_loop(self, env_obs_dict):
        ...
        for _ in tqdm(range(self.env.max_episode_length), desc="Open Loop Rollout"):
            ...
            nn_pcd_obs = self._update_mpinets_robot_pcd_observation(nn_pcd_obs, current_robot_pcd)
    ...
    def _prepare_mpinets_observation(
        self,
    ...
    def _update_mpinets_robot_pcd_observation(self, nn_pcd_obs, current_robot_pcd):
        nn_pcd_obs[:, 0:self.num_robot_points, 0:3] = current_robot_pcd
        return nn_pcd_obs
