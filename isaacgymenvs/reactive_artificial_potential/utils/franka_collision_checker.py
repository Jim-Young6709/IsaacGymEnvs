...
from .geometry import TorchCuboids, TorchSpheres


gpu_fk_sampler = FrankaSampler("cuda:0", use_cache=False, num_fixed_points=4096)
    ...
    def torch_spheres_cuboids(self, configs: torch.Tensor):
        ...
        fk_links = torch.cat(
            [
                ...
                link_transforms[:, 5:6].repeat(1, 3, 1, 1),
                link_transforms[:, 6:7].repeat(1, 5, 1, 1),
                link_transforms[:, 8:9].repeat(1, Mc + 22, 1, 1),
            ],
            dim=1,
        )
